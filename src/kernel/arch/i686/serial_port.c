...
#include "include/serial_port.h"
 ...
 enum BaudRate { Baud_115200 = 1, Baud_57600, Baud_19200, Baud_9600 };
   enum BaudRate divisor = Baud_115200;
    ...
    {
        i686_outb(SERIAL_LINE_COMMAND_PORT(com),
             SERIAL_LINE_ENABLE_DLAB);
        i686_outb(SERIAL_DATA_PORT(com),
             (divisor >> 8) & 0x00FF);
        i686_outb(SERIAL_DATA_PORT(com),
             divisor & 0x00FF);
    }
        ...
        void serial_configure_line(unsigned short com)
    {
        ...
        i686_outb(SERIAL_LINE_COMMAND_PORT(com), 0x03);
    }
    
    
        void serial_configure_fifo_buffer(unsigned short com) {

  i686_outb(SERIAL_FIFO_COMMAND_PORT(com), 0xC7);
}
...
void serial_configure_modem(unsigned short com) {
  ...
  i686_outb(SERIAL_MODEM_COMMAND_PORT(com), 0x03);
}
    
    
        int serial_is_transmit_fifo_empty(unsigned int com)
    {
        return i686_inb(SERIAL_LINE_STATUS_PORT(com)) & 0x20;
    }
    
    
      void serial_write_byte(unsigned short port, char byteData) {
  i686_outb(port, byteData);
}  
